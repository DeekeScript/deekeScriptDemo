...
export const Common = {
    ...
    sleep(time) {
        time > 100 ? Log.log("js休眠时间：" + time) : null;
        System.sleep(time);
    },
    ...
    closeAlert() {
        ...
        while (true) {
            ...
            try {
                let a = null;
                if (new UiSelector().text("稍后").exists()) {
                ...
                if (new UiSelector().text("取消").clickable(true).exists()) {
                    a = new UiSelector().text("取消").clickable(true).filter((v) => {
                        return v && v.bounds() && v.bounds().top > 0 && v.bounds().height() + v.bounds().top < Device.height() && v.bounds().width() > 0 && v.bounds().height() > 0;
                    }).findOne();//id=b5c  夜晚提示早点休息
                }

                if (new UiSelector().text("拒绝").desc('拒绝').exists()) {
